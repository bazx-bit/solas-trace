...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let trace_id = format!("trace-rust-agno_tool_agent-{}", Uuid::new_v4().simple());
    let root_span_id = format!("span-rust-{}", &Uuid::new_v4().simple().to_string()[..6]);

    println!("Starting Rust Agno Tool Agent Agent Simulation...");
            ...
            "scopeSpans": [{
                "spans": [{
                    ...
                    "attributes": [
                        ...
                        { "key": "framework", "value": { "stringValue": "rust-agno_tool_agent" } },
