...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let trace_id = format!("trace-rust-microsoft_agent_framework-{}", Uuid::new_v4().simple());
    let root_span_id = format!("span-rust-{}", &Uuid::new_v4().simple().to_string()[..6]);

    println!("Starting Rust Microsoft Agent Framework Agent Simulation...");
        ...
        "resourceSpans": [{
            "scopeSpans": [{
                "spans": [{
                    "traceId": trace_id,
                    "spanId": root_span_id,
                    "name": "Microsoft Agent Framework Agent",
                    ...
                    "attributes": [
                        ...
                        { "key": "framework", "value": { "stringValue": "rust-microsoft_agent_framework" } },
