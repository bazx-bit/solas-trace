...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let trace_id = format!("trace-rust-langgraph_code_agent-{}", Uuid::new_v4().simple());
    let root_span_id = format!("span-rust-{}", &Uuid::new_v4().simple().to_string()[..6]);

    println!("Starting Rust Langgraph Code Agent Agent Simulation...");
        ...
        "resourceSpans": [{
            "scopeSpans": [{
                "spans": [{
                    "traceId": trace_id,
                    "spanId": root_span_id,
                    "name": "Langgraph Code Agent Agent",
                    ...
                    "attributes": [
                        ...
                        { "key": "framework", "value": { "stringValue": "rust-langgraph_code_agent" } },
