...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let trace_id = format!("trace-rust-anthropic_tool_agent-{}", Uuid::new_v4().simple());
    let root_span_id = format!("span-rust-{}", &Uuid::new_v4().simple().to_string()[..6]);

    println!("Starting Rust Anthropic Tool Agent Agent Simulation...");
        ...
        "resourceSpans": [{
            "scopeSpans": [{
                "spans": [{
                    "traceId": trace_id,
                    "spanId": root_span_id,
                    "name": "Anthropic Tool Agent Agent",
                    ...
                    "attributes": [
                        ...
                        { "key": "framework", "value": { "stringValue": "rust-anthropic_tool_agent" } },
