...
async function runTsAgent() {
  const traceId = `trace-ts-anthropic-${crypto.randomBytes(4).toString('hex')}`;
  const rootSpanId = `span-ts-${crypto.randomBytes(3).toString('hex')}`;
  
  console.log(`Starting TS Anthropic Agent...`);
      ...
      scopeSpans: [{
        spans: [{
          ...
          attributes: [
            ...
            { key: "framework", value: { stringValue: "typescript-anthropic" } }
          ]
        }]
      }]
    }]
