...
async function runTsAgent() {
  const traceId = `trace-ts-deepagents-${crypto.randomBytes(4).toString('hex')}`;
  const rootSpanId = `span-ts-${crypto.randomBytes(3).toString('hex')}`;
  
  console.log(`Starting TS Deepagents Agent...`);
      ...
      scopeSpans: [{
        spans: [{
          ...
          attributes: [
            ...
            { key: "framework", value: { stringValue: "typescript-deepagents" } }
          ]
        }]
      }]
    }]
