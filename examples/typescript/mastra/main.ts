...
async function runTsAgent() {
  const traceId = `trace-ts-mastra-${crypto.randomBytes(4).toString('hex')}`;
  const rootSpanId = `span-ts-${crypto.randomBytes(3).toString('hex')}`;
  
  console.log(`Starting TS Mastra Agent...`);
      ...
      scopeSpans: [{
        spans: [{
          ...
          attributes: [
            ...
            { key: "framework", value: { stringValue: "typescript-mastra" } }
          ]
        }]
      }]
    }]
