...
async function runTsAgent() {
  const traceId = `trace-ts-bedrock-${crypto.randomBytes(4).toString('hex')}`;
  const rootSpanId = `span-ts-${crypto.randomBytes(3).toString('hex')}`;
  
  console.log(`Starting TS Bedrock Agent...`);
      ...
      scopeSpans: [{
        spans: [{
          ...
          attributes: [
            ...
            { key: "framework", value: { stringValue: "typescript-bedrock" } }
          ]
        }]
      }]
    }]
