...
async function runTsAgent() {
  const traceId = `trace-ts-openai-${crypto.randomBytes(4).toString('hex')}`;
  const rootSpanId = `span-ts-${crypto.randomBytes(3).toString('hex')}`;
  
  console.log(`Starting TS Openai Agent...`);
      ...
      scopeSpans: [{
        spans: [{
          ...
          attributes: [
            ...
            { key: "framework", value: { stringValue: "typescript-openai" } }
          ]
        }]
      }]
    }]
