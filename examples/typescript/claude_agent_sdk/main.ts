...
async function runTsAgent() {
  const traceId = `trace-ts-claude_agent_sdk-${crypto.randomBytes(4).toString('hex')}`;
  const rootSpanId = `span-ts-${crypto.randomBytes(3).toString('hex')}`;
  
  console.log(`Starting TS Claude Agent Sdk Agent...`);
    ...
    resourceSpans: [{
      scopeSpans: [{
        spans: [{
          traceId,
          spanId: rootSpanId,
          name: "Claude Agent Sdk TS Agent",
          ...
          attributes: [
            ...
            { key: "framework", value: { stringValue: "typescript-claude_agent_sdk" } }
          ]
        }]
      }]
    }]
